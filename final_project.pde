...
//---------------------sounds--------------------------//
gunArray[gunNow] = new Gun(gunNow); //預設使用第1隻槍(必須在sound後面 因為音效要先近來 槍才會載入音效)
}
...
void keyPressed() {
  ...
  if(key == ' '){
    if(gunArray[gunNow].bulletNow>0){
      hero.shooting = true;
      if(Timer(gunArray[gunNow].shootRate,hero.shooting)==true){
        gunArray[gunNow].bulletNow --;
        hero.shooting = true;
      }else{hero.shooting = false;}
      if(hero.shooting == true){
        println(gunNow);
        println(gunArray[gunNow].gunSounds);
