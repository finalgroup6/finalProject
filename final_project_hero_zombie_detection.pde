...
void draw(){
  ...
  for (int i=0; i<zombieNow; i++){
    println(zombieArray[i].zombieHitPassedTime+"  "+zombieArray[i].zombieHitSavedTime);
    //zombieArray[i].zombieHitPassedTime = millis()-zombieArray[i].zombieHitSavedTime;
    if(zombieArray[i].x != width && zombieArray[i].y != height){
      if(zombieArray[i].shooted==false){zombieArray[i].move();}
      zombieArray[i].display();
      zombieArray[i].hpCheck();
      if(hero.shooting){
        if(zombieArray[i].shooted(hero.nowDirectionNum, i)){
        //殭屍被打到的硬直  
          zombieArray[i].zombieHitSavedTime = millis();
        //殭屍被打到的硬直  
     ...
     //殭屍被打到的硬直   
      if(zombieArray[i].zombieHitimer(300,zombieArray[i].shooted)){
